...
/*
        Function execute the reduction of entry array addr.It returns the number
   of entries in the final array.
        PARAM: addr array: contains entries to be optimized
        PARAM: len: number of entries into addr array.
        PARAM: do_sort: if 0, sort will NOT be perfomed. Be carefull: optimizing
   a non-sorted input will produce unpredictable results
        NOTE3: resulting array may contains entries with prefix bigger than
   EXPANDED_PREFIX. Those are logical pointers: expanded_list[entry.network]
   (array returned by apply_whitelist function) is the starting position and the
   leght is entry.prefix- EXPANDED_PREFIX
*/
...
/*
        ...
        Returns number of element into output array expanded_list (that can be
   reassigned to allow array extension).
        Function allocate more memory if needed. It does NOT deallocate any if
   the result is smaller.
        PARAM: *entry_list: pointer to input/output param entry list.
        PARAM: **expanded_list: pointer to output param entry list, used to
   store expanded networks.
        ...
        PARAM: size_expanded: return paramether that contains memory size of the
   output expanded_list array
        PARAM: do_sort: if 0, sort will NOT be perfomed on both entry_list and
   white. Be carefull: optimizing a non-sorted input will produce unpredictable
   results
        NOTE1: both arrays entry_list and white have to be sorted with
   sort_entries() function BEFORE calling this.function OR paramether do_sort
   needs to be set to non zero. Unpredictable results if not
        NOTE2: resulting array may contains entries with INVALID_PREFIX value
   prefix. This means entry is invalid and it doesn't have to be taken into
   account.
        NOTE3: resulting array may contains entries with prefix bigger than
   EXPANDED_PREFIX. Those are logical pointers: expanded_list[entry.network] is
   the starting position and the leght is entry.prefix- EXPANDED_PREFIX
        NOTE4: resulning array is ordered apart from invalid entries (the ones
   with INVALID_PREFIX value in prefix)
*/
