...
void print_addresses(STREAM f, struct entry *addr, int size,
                     struct entry *expanded, int level);
...
/*
        ...
        - a1.network is equal to a2.network but a1.prefix is bigger than
   a2.prefix
...
static void expand(struct entry **expanded_list, struct entry to_expand,
                   struct entry *white, unsigned int len_white, unsigned int n,
                   unsigned int *current_position,
                   unsigned int *size_expanded) {
...
/* we need to increase expanded_list array size */
    ...
    print_debug(STDOUT, debug_buf,
                "BEFORE realloc: size_expanded: %u, MEM: %u\n", *size_expanded,
    ...
    *size_expanded += BUCKET;
    ...
    result = *expanded_list =
        realloc(*expanded_list, (*size_expanded) * sizeof(struct entry));
    if (result == NULL) {
  ...
  while (first <= last) {
    ...
    tmp_big.network =
        to_expand.network +
        (0x1 << (32 - tmp_small.prefix)); /*last network bit set to 1 */

    if ((white[0].network & TONETMASK(tmp_small.prefix)) == tmp_small.network) {
      /* conflicting whith tmp_small. */
      i = 1;
      /* search for first conflicting position with tmp_big (it cant be the
       * first) */
      while ((i < len_white) &&
      ...
      while (
          (i < len_white) &&
          ((white[i].network & TONETMASK(tmp_big.prefix)) == tmp_big.network)) {
        i++;
      }
      last_conflict = i;
      /* check if there is at least one conflicting with tmp_big network into
       * whitelist */
      if (first_conflict != len_white) {
        if (white[first_conflict].prefix != tmp_big.prefix) {
          ...
          result[last].prefix =
              EXPANDED_PREFIX + white[first_conflict].prefix - tmp_big.prefix;

          expand(expanded_list, tmp_big, &white[first_conflict],
                 last_conflict - first_conflict,
                 white[first_conflict].prefix - tmp_big.prefix,
                 current_position, size_expanded);
        } else {
#ifdef LIBRARY_DEBUG
      ...
      i = 1;
      /* search for first conflicting position with tmp_small (it cant be the
       * first)*/
      while ((i < len_white) &&
             !((white[i].network & TONETMASK(tmp_small.prefix)) ==
               tmp_small.network)) {
        i++;
      }
      first_conflict = i;
      ...
      /* check if there is at least one conflicting with tmp_small network into
       * whitelist */
      if (first_conflict != len_white) {
        if (white[first_conflict].prefix != tmp_small.prefix) {
          ...
          result[first].prefix =
              EXPANDED_PREFIX + white[first_conflict].prefix - tmp_small.prefix;

          expand(expanded_list, tmp_small, &white[first_conflict],
                 last_conflict - first_conflict,
                 white[first_conflict].prefix - tmp_small.prefix,
                 current_position, size_expanded);
        } else {
#ifdef LIBRARY_DEBUG
...
#ifdef LIBRARY_DEBUG
  print_debug(STDOUT, debug_buf,
              "EXPAND END. Local position %d, current position %u:\n", n,
              *current_position);
  print_debug(STDOUT, debug_buf, "\n");
#endif
}
...
/*
        Please see cidrmerge.h
*/
unsigned int apply_whitelist(struct entry *addr, struct entry **expanded_list,
                             struct entry *white, unsigned int len1,
                             unsigned int len2, unsigned int *size_expanded,
                             int do_sort) {
  ...
  while ((i1 < len1) && (i2 < len2)) {
    ...
    if ((addr[i1].prefix <= 32) &&
      ...
      if (addr[i1].prefix < white[i2].prefix) {
        ...
        invalid = i1 + 1;
        while ((invalid < len1) &&
               (addr[invalid].network & supermask) ==
...
/*address is already present in the expanded network, just drop it*/
        ...
        invalid -=
            i1 + 1; /*invalid represents the number of invalidated positions*/
        ...
        tmp = i2;
        i2 += 1;
        while ((i2 < len2) && ((addr[i1].network & supermask) ==
                               (white[i2].network & supermask))) {
          i2++;
        }
        ...
        print_debug(
            STDOUT, debug_buf,
            "EXPAND expanded_index %u whitelist elements: %u num expand %d\n",
        ...
        expand(expanded_list, tmp_entry, &(white[tmp]), i2 - tmp,
               white[tmp].prefix - tmp_entry.prefix, &expanded_index,
               size_expanded);
...
#ifdef LIBRARY_DEBUG
        print_debug(STDOUT, debug_buf,
                    "END MAIN EXPAND expanded_index %u i1: %u i2: %u\n",
...
/*just invalidating entry i1*/
    ...
    } else if ((addr[i1].prefix == INVALID_PREFIX) ||
               (addr[i1].network & supermask) <=
                   (white[i2].network & supermask)) {
      i1++;
    } else {
      i2++;
    ...
    if (addr[cur].prefix >= EXPANDED_PREFIX) {
      ...
      if ((addr[cur].prefix <= 32) &&
        ...
        if ((addr[i].prefix == addr[cur].prefix) &&
            ((addr[i].network & tmp_net) == (addr[cur].network & tmp_net))) {
#ifdef LIBRARY_DEBUG
